...
class RegistroCreacionEmpresa(models.Model):
    _name = 'juancarlosodoo.registro_creacion_empresa'
...
class juancarlosodoo_empresas_contratadoras(models.Model):
     ...
     def _get_show_task(self):
          param = self.env['ir.config_parameter'].sudo().get_param('juancarlosodoo.show_task')
          return param.lower() == 'true' if param else False
     
     def _get_dropdown_field(self):
          param = self.env['ir.config_parameter'].sudo().get_param('juancarlosodoo.dropdown_field')
          return param
     ...
     def create(self, values):
        res = super(juancarlosodoo_empresas_contratadoras, self).create(values)

        # Crear registro de creación de empresa
        nuevo1 = self.env['juancarlosodoo.registro_creacion_empresa'].create({
     ...
     def write(self, values):
        res = super(juancarlosodoo_empresas_contratadoras, self).write(values)

        # Crear registro de creación de empresa
        nuevo2 = self.env['juancarlosodoo.registro_creacion_empresa'].create({
            'user': self.env.user.name,
...
class ResConfigSettings(models.TransientModel):
    ...
    show_task = fields.Boolean(string='ver tareas', config_parameter="juancarlosodoo.show_task")
    dropdown_field = fields.Selection([
        ('America', 'America'),
        ('España', 'España'),
        ('España 2', 'España 2'),
    ], string='Dropdown Field', config_parameter="juancarlosodoo.dropdown_field")
